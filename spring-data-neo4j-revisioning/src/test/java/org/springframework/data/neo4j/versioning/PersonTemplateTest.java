...
public class PersonTemplateTest {
    ...
    public void testEntityCountLastesRevisionManual() {
        assertEquals(8, template.count(Person.class, versionManager.getCurrentRevisionNumber()));
    }
    ...
    public void renameMultipleTimes() {
        ...
        boolean found = false;
        List<Person> all = new ArrayList<Person>(IteratorUtil.asCollection(template.findAll(Person.class)));
        for (Person p : all) {
        ...
        found = false;
        all = new ArrayList<Person>(IteratorUtil.asCollection(template.findAll(Person.class, fourth)));
        for (Person p : all) {
        ...
        found = false;
        all = new ArrayList<Person>(IteratorUtil.asCollection(template.findAll(Person.class, third)));
        for (Person p : all) {
        ...
        found = false;
        all = new ArrayList<Person>(IteratorUtil.asCollection(template.findAll(Person.class, second)));
        for (Person p : all) {
            if (p.getName().equals("Pluto")) {
                found = true;
            }
        ...
        found = false;
        all = new ArrayList<Person>(IteratorUtil.asCollection(template.findAll(Person.class, first)));
        node = null;
        for (Person p : all) {
            if (p.getName().equals("Donald")) {
                found = true;
    ...
    public void testDeleteAllAndGoBack() {
        ...
        for (Person p : template.findAll(Person.class, versionManager.getCurrentRevisionNumber() - 1)) {
            beforeAfter.add(p);
        }
